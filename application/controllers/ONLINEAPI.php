...
$this->agentMsisdn = $json['CUSTOMER_MSISDN'];
$this->requestmode = "THIRDPARTY";
$id = $this->EncashServices->_getAgentID($this->agentMsisdn);
$this->agent_id = $id['id'];
if($service == "AIRTIME"){
  $this->operator = $json['OPERATOR'];
$this->_processAirtime($txnReference,$device,$amount,$this->operator);
...
function _processAirtime($refID,$msisdn,$amount,$operator){
...
$txn = array (
            ...
            "ERROR_DESC" => "SUBMITED",
            "TXN_ROUTE" =>$route,
            "TXN_MOBILE_NUMBER"=>$notifymsisdn
...
$this->EncashServices->_recordTransaction($table,$txnLog);
        ...
        $this->EncashServices->_updateTransaction($data,$txnid, $this->agentMsisdn);
